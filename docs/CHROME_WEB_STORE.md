...
- The existing `answer-clipper` Google project now displays **In production**. No new project or OAuth client was created for this transition. Branding verification was started from **Branding > Information and summary > Verify branding**; sensitive-scope approval still needs to be completed.
...
4. Complete branding verification/publication and sensitive Google Docs scope verification. **In production** is not **Verified**; the dashboard still shows a 100-user cap for unapproved scopes.
...
6. Two store-sized screenshots and reviewer instructions have been saved. Complete the three privacy certifications with the maintainer's confirmation. Update the reviewer instructions' temporary Google release-gate note after real authorization is ready.
