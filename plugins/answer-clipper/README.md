...
2. Clearly identify the text to save in ChatGPT so `add_answer_clip` can add it and open the workspace.

The macOS app or Chrome extension can later call `POST /api/clips` to synchronize a real text selection into the same draft while keeping annotation and organization inside the widget.

## Data and deployment

The default data file is `data/draft.json`. Set `ANSWER_CLIPPER_DATA_FILE` to use another path. `HOST` and `PORT` control the listening address and port.

This version is a single-user, local-first prototype. Before a public deployment, add authentication, per-user isolation, a persistent database, CSRF protection, and abuse controls. Never expose one default draft to multiple users.
